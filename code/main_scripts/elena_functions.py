...
def plot_training_sample(image, labdat, segments): # plotting a training sample of RF classifier
    ...
    water_training_mask = np.isin(segments,segments_ids[labdat.segnum[labdat.label == "water_wetsedge"]])
    veg_training_mask = np.isin(segments,segments_ids[labdat.segnum[labdat.label == "shrubs"]])
    soil_training_mask = np.isin(segments,segments_ids[labdat.segnum[labdat.label == "moss_sedge"]])
...
def plot_training_sample_spp(image, labdat, segments, SLIC_segm): # plotting a training sample of RF classifier
    ...
    veg_training_mask = np.isin(segments,segments_ids[labdat.segnum[labdat.label == "shrubs"]])
    soil_training_mask = np.isin(segments,segments_ids[labdat.segnum[labdat.label == "moss_sedge"]])
    veg_tr_mask = np.isin(SLIC_segm,np.unique(SLIC_segm)[labdat.SLIC_segnum[labdat.label == "shrubs"]])
    soil_tr_mask = np.isin(SLIC_segm,np.unique(SLIC_segm)[labdat.SLIC_segnum[labdat.label == "moss_sedge"]])
...
def plot_clfres(prlab, msp3, name_im, path_result, save = True): # plotting and saving the classification result
    water_mask = prlab == "OpenWater"
    shrub_mask = prlab == "Shrubs"
    dry_mask = prlab == "Dry"
    wet_mask = prlab == "Wet"
    str_mask = prlab == "Structures"
    
    masked_image = msp3.copy()
    masked_image[shrub_mask] = [0, 1, 0]
    masked_image[water_mask] = [0, 0, 1]
    masked_image[dry_mask] = [0.7, 0.3, 0.2] 
    masked_image[wet_mask] = [0.4, 0.8, 0.7]
    masked_image[str_mask] = [1, 1, 1]
    fig, (ax0, ax1) = plt.subplots(nrows=1, ncols=2, figsize=(15,30))
    ax0.imshow(msp3)
